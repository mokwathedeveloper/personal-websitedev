...
import { ExternalLink, Github, Code2, Sparkles, Star, GitFork, Loader2 } from "lucide-react"
import { PROJECTS, CONTENT, CATEGORY_ICONS, SITE_CONFIG } from "@/lib/data"
...
export function Projects() {
  ...
  React.useEffect(() => {
    const fetchProjects = async () => {
      ...
      const githubRepos = await getUserRepos(username)
      
      // Combine manual projects with GitHub repos if desired, 
      // or just use GitHub repos. For "full automation", we'll use GitHub repos.
      // We can also merge them by title if we want to keep manual descriptions.
      setAllProjects(githubRepos)
      setIsLoading(false)
    }
    fetchProjects()
  }, [])
    ...
    <Section id="projects" className="bg-background pt-24 lg:pt-32" fullHeight>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs font-heading">Fetching masterpieces...</p>
        </div>
      ) : (
        <>
          <FadeIn delay={0.2}>
                    ...
                    <SpotlightCard className="h-full flex flex-col group transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-2 border-primary/5 bg-card/30">
                      <div className="relative aspect-[16/10] overflow-hidden bg-muted/20">
                        ...
                        <div className="absolute top-4 left-4 z-10">
                          <Badge variant="secondary" className="backdrop-blur-md bg-background/60 shadow-sm border-primary/10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 text-foreground">
